...
public class Human {
    ...
    boolean makeMove(char c){
        ...
        switch(c){
            case 'e' -> log.add("Human moved East");
            case 's' -> log.add("Human moved South");
            case 'n' -> log.add("Human moved North");
            case 'w' -> log.add("Human moved West");
        }
    ...
    void use(String itemName){
        ...
        for(Item i : inventory){
            if(itemName.contentEquals(i.getName())){
                item = i;
                break;
            }
        }
        ...
        switch (item.getType()) {
            ...
            case 2 -> {
                setCharmType(1);
                setCharmStrength(item.getStrength());
                log.add("Human donned the pendant.");
            }
            case 3 -> {
                setCharmType(2);
                setCharmStrength(item.getStrength());
    ...
    private void combat(Goblin goblin){
        ...
        while(this.getHealth() > 0 && goblin.getHealth() > 0){
            ...
            if(damage == 0){
                log.add("The human misses.");
            }else if(damage > .7 * this.getStrength()){
                log.add("The human hits hard for " + damage + ".");
            }else {
                log.add("The human hits for " + damage + ".");
            }
            ...
            if(goblin.getHealth() > 0) {
                ...
                if(damage == 0){
                    log.add("The goblin misses.");
                }else if(damage > .7 * goblin.getStrength()){
                    log.add("The goblin hits hard for " + damage + ".");
                }else {
                    log.add("The goblin hits for " + damage + ".");
                }

            } else{
                log.add("The goblin has died.");
        ...
        if(this.getHealth() > 0){
            log.add("The human won with " + this.getHealth() + " health remaining.");
            if(getCharmType() == 2){
                baseStrength++;
    ...
    private void loot(Treasure treasure){
        if(treasure != null) {
            ...
            for (Item i : treasure.getInventory()) {
                log.add("The human looted " + i.getName() + ".");
                if(i.getType() == 0){
                    health += i.getStrength();
                    log.add("The human ate the " + i.getName() + " bringing their health to " + health);
                    consumed.add(i);

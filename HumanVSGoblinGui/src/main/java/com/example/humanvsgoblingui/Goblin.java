...
public class Goblin {
    ...
    private void combat(Human human) {
        ...
        while (this.getHealth() > 0 && human.getHealth() > 0) {
            ...
            if (damage == 0) {
                human.getLog().add("The goblin misses.");
            } else if (damage > .7 * this.getStrength()) {
                human.getLog().add("The goblin hits hard for " + damage + ".");
            } else {
                human.getLog().add("The goblin hits for " + damage + ".");
            }
            ...
            if (human.getHealth() > 0) {
                ...
                if (damage == 0) {
                    human.getLog().add("The human misses.");
                } else if (damage > .7 * human.getStrength()) {
                    human.getLog().add("The human hits hard for " + damage + ".");
                } else {
                    human.getLog().add("The human hits for " + damage + ".");
                }

            } else {
                human.getLog().add("The human has died.");
            }
        ...
        if(this.getHealth() <= 0){
            ...
            if(this.loot != null) {
                ...
                for (Item i : getLoot().getInventory()) {
                    human.getLog().add("The human looted " + i.getName() + ".");
                    if(i.getType() == 0){
                        human.setHealth(human.getHealth() + i.getStrength());
                        human.getLog().add("The human ate the " + i.getName() + " bringing their health to " + human.getHealth());
                        consumed.add(i);
            ...
            human.getLog().add("The human has won with " + human.getHealth() + " remaining.");
